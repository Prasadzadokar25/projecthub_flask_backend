...
@bank_account_bp.route('/add-bank-account', methods=['POST'])
def add_bank_account():
    data = request.get_json()
    obj =BackAcountController()
    return obj.add_bank_account(data)

@bank_account_bp.route('/accounts', methods=['GET'])
def get_accounts_for_user():
    user_id = request.args.get('user_id')
    obj =BackAcountController()
    return obj.get_accounts_for_user(user_id)

@bank_account_bp.route('/set-primary-account/<int:user_id>/<int:account_id>', methods=['PUT'])
